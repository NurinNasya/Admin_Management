...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['leave_type'])) {
  ...
  if ($controller->saveLeaveApplication($leave_data)) {
    $_SESSION['flash'] = ['type' => 'success', 'message' => 'Leave application successfully sent.'];
...
<head>
  ...
  <style>
    .status-supported { color: #2dce89; font-weight: bold; }
    .form-control-label:after { content: " *"; color: red; }
    .employee-info-card { background: white !important; color: #495057 !important; border: 1px solid #dee2e6 !important; }
    .leave-type-radio label { font-weight: normal !important; }
    .action-buttons { display: flex; gap: 5px; }
    .status-approved { color: #11cdef; font-weight: bold; }
    .status-pending { color: #fb6340; font-weight: bold; }
    .status-rejected { color: #f5365c; font-weight: bold; }
    .version-badge { background-color: #5e72e4; color: white; padding: 3px 10px; border-radius: 15px; font-size: 12px; float: right; }
    .leave-type-radio { display: flex; flex-direction: column; gap: 8px; }
    .leave-type-radio label { display: flex; align-items: center; gap: 8px; cursor: pointer; }
    .leave-type-radio input[type="radio"] { margin: 0; }
    .action-dropdown { position: relative; display: inline-block; }
    .dropdown-content { display: none; position: absolute; background-color: #f9f9f9; min-width: 160px; box-shadow: 0px 8px 16px 0px rgba(0,0,0,0.2); z-index: 1; right: 0; border-radius: 8px; }
    ...
    .reminder-box { background-color: #fff3cd; border: 1px solid #ffeaa7; padding: 15px; border-radius: 8px; margin-bottom: 20px; }
    .reminder-box h4 { color: #856404; margin-bottom: 10px; }
    .employee-info-row { display: flex; margin-bottom: 10px; }
    .employee-info-row label { font-weight: bold; width: 120px; margin-right: 10px; }
    .employee-info-row span { flex: 1; }
    .user-name-display { background-color: #e9f7ef; padding: 5px 15px; border-radius: 20px; color: #2dce89; font-weight: 600; }
    .low-balance { background-color: #fff3cd; }
    .low-balance td { font-weight: bold; color: #856404; }
  </style>
</head>
                        ...
                        <tr>
                          ...
                          <td>
                            ...
                            <?php if (!empty($app['hr_checked_at'])): ?>
                              <p class="text-xs text-secondary mb-0"><?php echo date('d/m/Y H:i', strtotime($app['hr_checked_at'])); ?></p>
                            <?php endif; ?>
                          </td>
                          <td>
                            <div class="action-buttons">
                              <?php if (!empty($app['attachment'])): ?>
                                <a href="javascript:void(0);" onclick="viewAttachment(<?php echo $app['id']; ?>)" class="text-secondary" title="Attachment">
                                  Lampiran
                                </a>
                                <?php if (($app['status'] ?? 'pending') == 'pending'): ?> | <?php endif; ?>
                              <?php endif; ?>
                              <a href="javascript:void(0);" onclick="viewInfo(<?php echo $app['id']; ?>)" class="text-info" title="Info" data-toggle="modal" data-target="#infoModal">
                                Information
                              </a>
                              <?php if (($app['status'] ?? 'pending') == 'pending'): ?>
                                | <a href="javascript:void(0);" onclick="updateApplication(<?php echo $app['id']; ?>)" class="text-warning" title="Update">
                                  Updated
                                </a>
                                | <a href="javascript:void(0);" onclick="deleteApplication(<?php echo $app['id']; ?>)" class="text-danger" title="Delete" data-toggle="modal" data-target="#deleteModal">
                                  Delete
                                </a>
                              <?php endif; ?>
                            </div>
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-3">
                      <div class="form-group">
                        ...
                        <input class="form-control" type="date" name="start_date" id="start_date" required min="<?php echo date('Y-m-d', strtotime('+3 days')); ?>">
                      </div>
                    </div>
                    <div class="col-md-3">
                      <div class="form-group">
                        <label class="form-control-label">End Date </label>
                        <input class="form-control" type="date" name="end_date" id="end_date" required min="<?php echo date('Y-m-d', strtotime('+3 days')); ?>">
                      </div>
                    </div>
                    <div class="col-md-3">
                      <div class="form-group">
                        <label class="form-control-label">Total Day </label>
                        <input class="form-control" type="number" name="total_days" id="total_days" step="0.5" min="0.5" placeholder="1.0" required readonly>
                      </div>
                    </div>
                  </div>
                          ...
                          <?php foreach ($dynamic_leave_types as $leave_type):
                            ...
                            <label>
                              <input type="radio" name="leave_type" value="<?php echo htmlspecialchars($type_name); ?>"
                                data-id="<?php echo $type_id; ?>" data-quota="<?php echo $quota; ?>"
              ...
              <div class="card-body">
                ...
                <form method="POST" action="../Controller/LeaveFormController.php?action=update_application" id="editLeaveForm" enctype="multipart/form-data">
                  ...
                  <div class="row">
                    <div class="col-md-3">
                      <div class="form-group">
                        <label class="form-control-label">Application date <span class="text-danger">*</span></label>
                        <input class="form-control" type="date" name="application_date" value="<?php echo date('Y-m-d', strtotime($application['created_at'])); ?>" required readonly>
                      </div>
                    </div>
                    <div class="col-md-3">
                      <div class="form-group">
                        <label class="form-control-label">Start date <span class="text-danger">*</span></label>
                        <input class="form-control" type="date" name="start_date" id="edit_start_date" value="<?php echo $application['start_date']; ?>" required>
                      </div>
                    </div>
                    <div class="col-md-3">
                      <div class="form-group">
                        <label class="form-control-label">End Date <span class="text-danger">*</span></label>
                        <input class="form-control" type="date" name="end_date" id="edit_end_date" value="<?php echo $application['end_date']; ?>" required>
                      </div>
                    </div>
                    <div class="col-md-3">
                      <div class="form-group">
                        <label class="form-control-label">Total Day <span class="text-danger">*</span></label>
                        <input class="form-control" type="number" name="total_days" id="edit_total_days" step="0.5" min="0.5" value="<?php echo $application['total_days']; ?>" required readonly>
                      </div>
                    </div>
                  </div>
                  ...
                  <div class="row">
                      ...
                      <div class="form-group">
                        ...
                        <div class="leave-type-radio">
                          <?php foreach ($dynamic_leave_types as $leave_type):
                            ...
                            <label>
                              <input type="radio" name="leave_type" value="<?php echo htmlspecialchars($type_name); ?>" <?php echo $checked; ?>>
                              <?php echo htmlspecialchars($type_name); ?>
                            </label>
                          <?php endforeach; ?>
                        </div>
                    ...
                    <div class="col-md-4">
                      <div class="form-group">
                        <label class="form-control-label">Reason for requesting leave <span class="text-danger">*</span></label>
